import asyncio
from typing import Any, Dict, List, Optional

from prisma import Prisma
from prisma.errors import RecordNotFoundError

prisma = Prisma()


def _serialize_course(course: Any) -> Dict[str, Any]:
    return {
        "id": course.id,
        "name": course.name,
        "description": course.description,
        "semester": course.semester,
        "status": course.status,
        "created_at": course.created_at.isoformat() if course.created_at else None,
        "updated_at": course.updated_at.isoformat() if course.updated_at else None,
    }


async def _find_many(status: Optional[bool] = None) -> List[Dict[str, Any]]:
    await prisma.connect()
    try:
        where: Dict[str, Any] = {}
        if status is not None:
            where["status"] = status

        courses = await prisma.courses.find_many(where=where or None)
        return [_serialize_course(course) for course in courses]
    finally:
        await prisma.disconnect()


async def _find_one(course_id: int) -> Optional[Dict[str, Any]]:
    await prisma.connect()
    try:
        course = await prisma.courses.find_unique(where={"id": course_id})
        return _serialize_course(course) if course else None
    finally:
        await prisma.disconnect()


async def _create(data: Dict[str, Any]) -> Dict[str, Any]:
    await prisma.connect()
    try:
        course = await prisma.courses.create(data=data)
        return _serialize_course(course)
    finally:
        await prisma.disconnect()


async def _update(course_id: int, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    await prisma.connect()
    try:
        course = await prisma.courses.update(where={"id": course_id}, data=data)
        return _serialize_course(course)
    except RecordNotFoundError:
        return None
    finally:
        await prisma.disconnect()


async def _update_status(course_id: int, status: bool) -> Optional[Dict[str, Any]]:
    return await _update(course_id, {"status": status})


def find_many(status: Optional[bool] = None) -> List[Dict[str, Any]]:
    return asyncio.run(_find_many(status))


def find_one(course_id: int) -> Optional[Dict[str, Any]]:
    return asyncio.run(_find_one(course_id))


def create(data: Dict[str, Any]) -> Dict[str, Any]:
    return asyncio.run(_create(data))


def update(course_id: int, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    return asyncio.run(_update(course_id, data))


def update_status(course_id: int, status: bool) -> Optional[Dict[str, Any]]:
    return asyncio.run(_update_status(course_id, status))

